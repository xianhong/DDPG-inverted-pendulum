...
num_episodes = 800 #15000		# number of episodes
...
#
tf.add_to_collection("state_ph", state_ph)
tf.add_to_collection("is_training_ph", is_training_ph)
...
for ep in range(num_episodes):
    ...
    for t in range(max_steps_ep):
       ...
       #print("Action state predicted:",action_for_state)
       ...
       #print("Action state with added noise:",action_for_state)
       # take step
       ...
       if (total_steps%train_every == 0 and len(replay_memory) >= minibatch_size):
             ...
             _, _ = sess.run([critic_train_op, actor_train_op], 
                             feed_dict = {state_ph: np.asarray([elem[0] for elem in minibatch]),
...
episodes_int= sess.run("episodes:0").astype(int)
print(episodes_int)
checkpoint_file = os.path.join(log_dir, 'pendulum-model.ckpt')
saver.save(sess, checkpoint_file, global_step=episodes_int)
